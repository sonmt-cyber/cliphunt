...
export default function Settings(){
  ...
  const submit=async(e:FormEvent<HTMLFormElement>,url:string)=>{e.preventDefault();setMessage("");const f=new FormData(e.currentTarget);const body=Object.fromEntries(f);try{await api(url,{method:"POST",body:JSON.stringify(body)});e.currentTarget.reset();await refresh();setMessage("Đã lưu thành công.")}catch(err){setMessage((err as Error).message)}};
